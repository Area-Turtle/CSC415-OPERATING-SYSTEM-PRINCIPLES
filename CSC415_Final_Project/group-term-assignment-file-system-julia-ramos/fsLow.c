...
int initializePartition(int fd, uint64_t volSize, uint64_t blockSize) {
    ...
    fsync(fd);
    printf(
        "Created a volume with %llu bytes, broken into %llu blocks of %llu "
        "bytes.\n",
...
int startPartitionSystem(char* filename, uint64_t* volSize,
    ...
    if (accessRet == -1) {
        if (errno == ENOENT)  // The volume file does not exist, initialize it
        {
            ...
            // example 1000 0111 anded = 0 1010 1001 = 1000 when anded i.e. not
            // power of 2
    ...
    if ((buf->signature == PART_SIGNATURE) &&
        (buf->signature2 == PART_SIGNATURE2)) {
        *volSize = buf->volumesize;
        *blockSize = buf->blocksize;
        partInfop =
            malloc(sizeof(partitionInfo_t) + strlen(buf->volumeName) + 4);
        memcpy(partInfop, buf,
               sizeof(partitionInfo_t) + strlen(buf->volumeName) + 4);
    ...
    free(buf);
    if (retVal != PART_NOERROR) close(fd);
    return retVal;
}
...
uint64_t LBAread(void* buffer, uint64_t lbaCount, uint64_t lbaPosition) {
    ...
    lseek(partInfop->fd, fl.l_start, SEEK_SET);
    int reRead = read(partInfop->fd, buffer, fl.l_len);

    fl.l_type = F_UNLCK;
    fcntl(partInfop->fd, F_SETLKW, &fl);

    // return 0;
    return reRead / partInfop->blocksize;
}
