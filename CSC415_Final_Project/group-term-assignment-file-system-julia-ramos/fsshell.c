...
int displayFiles(fdDir *dirp, int flall, int fllong) {
    ...
    while (di != NULL) {
        if ((di->d_name[0] != '.') ||
            (flall))  // if not all and starts with '.' it is hidden
        {
            if (fllong) {
...
int cmd_ls(int argcnt, char *argvec[]) {
    ...
    optind = 0;
#else
    // "optind" is used between getopt() calls to get next argument for parsing
    // and should be initialized before each parsing loop.
    optind = 1;
#endif
    fllong = 0;
    flall = 0;
    ...
    while (1) {
        c = getopt_long(argcnt, argvec, "alh", long_options, &option_index);
    ...
    {
        char *path =
            fs_getcwd(cwd, DIRMAX_LEN);  // get current working directory
        fdDir *dirp;
...
#if (CMDMV_ON == 1)
    if (argcnt != 3) {
        printf("Usage: mv sourceFilename destDirectory\n");
        return -1;
    } else {
...
int cmd_pwd(int argcnt, char *argvec[]) {
    ...
    {
        printf(
            "An error occurred while trying to get the current working "
            "directory\n");
    } else {
        printf("%s\n", ptr);
    }
    ...
    for (i = 0; i < cmdLen; i++) {
        switch (cmd[i]) {
            case ' ':
                ...
                while (
                    cmd[i + 1] ==
                    ' ')  // null at end will prevent from overshooting string
                {
                    i++;
                }
            ...
            case DOUBLE_QUOTE:  // ignore everything till next quote (unless
                                // unterminated)
...
int main(int argc, char *argv[]) {
    ...
    // declare the file system array of size 10mil bytes of structs called
    // "block"s

    // Use index 0 of our array to hold our master boot record

    // contains head of our free space allocation linked list
